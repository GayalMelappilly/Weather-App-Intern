            ...
            return (
                <div className='h-28 p-2 bg-white bg-opacity-10 rounded-lg shadow-lg shadow-black-400 hover:bg-opacity-20 active:bg-opacity-30'>
                  <div className='flex'>
                    ...
                    <p key={index} className='pl-1 pt-4 font-normal font-lg'>{obj?.main.temp_min.toFixed(0)}° / {obj?.main.temp_max.toFixed(0)}°</p>
                  </div>
                  <p key={index} className='text-sm font-thin'>{toDate(obj?.dt_txt.slice(0, 14))}</p>
                </div>
            )
          }
        )}
